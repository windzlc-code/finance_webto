...
> 本文档不是“重新做 UI”的计划，而是“用现成前端模板承载 TFSE 项目闭环”的执行文档。核心工作是识别模板已有的页面、区块、交互、关键词栏位、图片占位和表单位置，然后把 TFSE 的金融便民定位、资料库结构、免费健检表单、Line 私域、SEO 内容、后台 CRM 和合规机制准确套进去。
