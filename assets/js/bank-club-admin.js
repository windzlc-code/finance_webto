(function () {
    ...
    function renderModule(data, leadResult) {
        ...
        root.innerHTML = [
            ...
            "<div><span>多站點管理</span><h2>Bank Club 主站</h2><p>主站核心內容、線索、內容資源與事件分析已接入当前 TFSE 後台，資料以 site_key=bank_club 分開管理。資料來源：" + escapeHtml(leadResult.mode || "localStorage") + "</p></div>",
            "<div class=\"bank-admin-actions\"><a href=\"bank-club/index.html\" target=\"_blank\" rel=\"noopener\">打開 Bank Club 前台</a><button type=\"button\" data-bank-export>匯出 Bank Club 資料</button></div>",
            "</div>",
            ...
            "<section><h3>站點設定</h3><ul class=\"bank-admin-settings\"><li><span>專員</span><b>" + escapeHtml(data.settings.specialistName) + "｜" + escapeHtml(data.settings.specialistTitle) + "</b></li><li><span>電話</span><b>" + escapeHtml(data.settings.mobile) + "</b></li><li><span>Email</span><b>" + escapeHtml(data.settings.email) + "</b></li><li><span>前台路徑</span><b>bank-club/index.html</b></li></ul></section>",
            "</div>"
        ].join("");
    }
